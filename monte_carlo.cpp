//Implementation of Hex Board Game using AI techniques

#include <iostream>
#include <cstring>

using std::cout;
using std::cin;
using std::endl;

//to change the board size
const int MIN_N = 3;        // Smallest board size
const int MAX_N = 26;	   // Largest board size (limit of alphanumeric notation)

enum State	         // board point or move state
{
	EMPTY = 0,              // empty board position
	VERT = 1,               // computer player (vertical = j)
	HORIZ = 2,               // human player (horizontal = i)
	NUM_STATES
};

static char* StateNames[NUM_STATES] =
{
	"Empty",
	"Vertical",
	"Horizontal",
};

struct Coordinates
{
	Coordinates(int i_ = -1, int j_ = -1) : i(i_), j(j_) {}

	int i;
	int j;
};

static const Coordinates nbor[6] =
{
	Coordinates(1, -1), Coordinates(1, 0), Coordinates(0, 1),
	Coordinates(-1, 1), Coordinates(-1, 0), Coordinates(0, -1)
};

// Blind row defense templates
//
// Key:
//     h = existing HORIZ piece
//     v = existing VERT piece
//     . = empty position
//     x = existing piece (VERT or HORZ)
//     - = don't care
//     H = last HORZ move
//     V = VERT's best reply
//     r = reentrant block on furthest connected piece (or adjacent if none)
//
// Note: more general situations should go last.
//
const int NUM_DEFENSE_TEMPLATES = 25;
static const char* DefenseTemplate[NUM_DEFENSE_TEMPLATES][2] =
{
	{ "H V - -", "- - - -" },	// acute blind corner
	{ "- - . H", "- - V -" },	// obtuse blind corner
	{ "- V H v", "- - - -" },
	{ "- v H V", "- - - -" },
	{ "- V H .", "h . . h" },
	{ "- . H V", "h . . h" },
	{ "v h H -", "V . h -" },
	{ "- H h v", "h . V -" },
	{ "- V H -", "h . h -" },
	{ "- - H V", "- h . h" },
	{ "- V H -", "- v - -" },
	{ "- H V -", "- v - -" },
	{ "- V H -", "- - h -" },
	{ "- H V -", "h - - -" },
	{ ". H h -", "V - - -" },
	{ "- h H .", "- - V -" },
	{ "- V H -", "h . - -" },
	{ "- H V -", "- . h -" },
	{ "- h H h", "- v V v" },
	{ "- h H h", "v V v -" },
	{ "- h H v", "- v V -" },
	{ "- v H h", "- V v -" },
	{ "- H r -", "h - - -" },
	{ "- r H -", "- - h -" },
	{ "- . H .", "- V . -" },
};


class HEX_Game
{
public:
	HEX_Game(int n_ = 11) : N(n_), SwapOption(true) {}
	~HEX_Game()	{}

	bool	Play(void);
private:
	bool	Move(State who);
	bool	GetUserMove(Coordinates& move) const;
	bool	Won(State who);
	bool	Won(State who, Coordinates const& from);
	void	RandomEmptyPosition(Coordinates& pos) const;
	void	ShowBoard(void) const;
	bool	IsValid(Coordinates const& pos) const;

	void	BestOpening(Coordinates& move) const;
	bool	GoodSwap(Coordinates const& move) const;
	bool	BestComputerMove(Coordinates& move) const;
	bool	DefendBlindRow(Coordinates& move) const;
	void	SpareMove(Coordinates& move) const;

private:
	State   Board[MAX_N][MAX_N];         // the board
	int     N;                          // current board size NxN
	int     NumMoves;                  // number of moves played
	Coordinates   Last;               // opponent's last move
	bool    SwapOption;              // whether the swap option is enabled
	bool    Swapped;             // whether a player swapped already
	State   WhoStarted;            // the one played first
	bool    Visit[MAX_N][MAX_N];  // temp
};

// Alternates: moves between players until the game is won
// Returns: whether user choose to continue playing

bool HEX_Game::Play(void)
{
	memset(Board, 0, sizeof(Board));
	NumMoves = 0;
	Swapped = false;

	// Determine who starts
	cout << "\nDo you want to start? [y/n/q]: ";
	char ch('n');
	cin >> ch;

	if (tolower(ch) == 'q')
		exit(1);

	WhoStarted = (tolower(ch) == 'y') ? HORZ : VERT;

	if (WhoStarted == HORZ)
		ShowBoard();

	// Play a game
	State who(WhoStarted);
	while (!Move(who))
		who = State((NumMoves + WhoStarted + 1) % 2 + 1);

	cout << "\nGame won by " << StateNames[who] << "!" << endl;
	cout << "\nPlay again on " << N << 'x' << N << "? [y/n]: ";
	cin >> ch;

	return (tolower(ch) == 'y');
}

// Determines and plays a move for the specfied player
// Returns: whether this move wins the game

bool HEX_Game::Move(State who)
{
	Coordinates move;
	bool did_swap = (who == HORZ) ? GetUserMove(move) : BestComputerMove(move);
	if (did_swap)
	{
		// 'who' chose to swap
		State opp = (who == VERT) ? HORZ : VERT;
		Board[Last.j][Last.i] = EMPTY;	// undo opening move
		Board[Last.i][Last.j] = who;	// redo its reflection
		ShowBoard();
		cout << endl << "Swap! " << StateNames[who][0] << " takes ";
		cout << char('A' + Last.j) << 1 + Last.i << "." << endl;

		WhoStarted = who;
		Swapped = true;
	}
	else
	{
		// Make the move
		Board[move.j][move.i] = who;
		NumMoves++;
		Last = move;

		// Show the result
		ShowBoard();
		cout << endl << StateNames[who][0] << " plays at ";
		cout << char('A' + move.i) << 1 + move.j << "." << endl;
	}

	return Won(who);
}

// Reads user's move from the keyboard (HORZ)
// Returns: whether user chose to swap

bool HEX_Game::GetUserMove(Coordinates& move) const
{
	char str[4];
	if (SwapOption && !Swapped && NumMoves == 1)
	{
		char ch('y');
		cout << endl << "Swap opening move? ";
		if (GoodSwap(Last))
			cout << "(I would) [y/n/q]: ";
		else
			cout << "(I wouldn't) [y/n/q]: ";
		cin >> ch;

		if (tolower(ch) == 'q')
			exit(2);

		if (tolower(ch) == 'y')
			return true;		// Swap
	}

	do {
		cout << endl << "Enter move [eg f6/q]: ";
		cin >> str;

		if (str[0] == 'q')
			exit(3);

		move.i = tolower(str[0]) - 'a';
		move.j = atoi(&str[1]) - 1;
	} while (!IsValid(move) || Board[move.j][move.i]);

	return false;
}

// Returns: whether the game has been won by the specified player

bool HEX_Game::Won(State who)
{
	memset(Visit, 0, sizeof(Visit));
	if (who == VERT)
	{
		for (int i = 0; i < N; i++)
		if (Board[0][i] == VERT && !Visit[0][i] && Won(VERT, Coordinates(i, 0)))
			return true;
	}
	else
	{
		for (int j = 0; j < N; j++)
		if (Board[j][0] == HORZ && !Visit[j][0] && Won(HORZ, Coordinates(0, j)))
			return true;
	}
	return false;
}

// Recursively examines neighbours of potential winning chains
// Returns: whether the game has been won by the specified player

bool HEX_Game::Won(State who, Coordinates const& from)
{
	if (who == HORZ && from.i == N - 1 || who == VERT && from.j == N - 1)
		return true;

	Visit[from.j][from.i] = true;		// this Coordinates has been visited

	for (int n = 0; n < 6; n++)
	{
		// Check neighbours for continuation of chain
		Coordinates to(from.i + nbor[n].i, from.j + nbor[n].j);
		if
			(
			IsValid(to) && Board[to.j][to.i] == who
			&&
			!Visit[to.j][to.i] && Won(who, to)
			)
			return true;
	}
	return false;
}

// Draws board in ASCII text format

void HEX_Game::ShowBoard(void) const
{
	//	Show alphabetical (HORZ) labels
	cout << endl << "  ";
	for (int i = 0; i < N; i++)
		cout << char('A' + i) << ' ';
	cout << endl;

	for (int j = 0; j < N; j++)
	{
		// Show numeric (VERT) labels
		for (int s = 0; s < j; s++)
			cout << ' ';
		if (j < 9)
			cout << ' ';
		cout << j + 1;

		// Show state of each board position along this row
		for (int i = 0; i < N; i++)
		if (Board[j][i] == VERT)
			cout << " V";
		else if (Board[j][i] == HORZ)
			cout << " H";
		else
			cout << " .";
		cout << endl;
	}
}

// Returns: whether pos is within the bounds of the board

bool
HEX_Game::IsValid(Coordinates const& pos) const
{
	return (pos.i >= 0 && pos.i < N && pos.j >= 0 && pos.j < N);
}

// Determines computer's best opening move (VERT)
void HEX_Game::BestOpening(Coordinates& move) const
{
	move = Coordinates(N - 1, 0);					// obtuse corner in blind row
}

// Returns: whether this opening move is worth swapping

bool
HEX_Game::GoodSwap(Coordinates const& move) const
{
	return (move.i == N - move.j - 1);		// swap along short diagonal
}

// Determines best move for computer (VERT)
// Returns:  true on swap, else false

bool
HEX_Game::BestComputerMove(Coordinates& move) const
{
	if (NumMoves == 0)
	{
		BestOpening(move);				// opening move
	}
	else if (NumMoves == 1 && SwapOption && !Swapped && GoodSwap(Last))
	{
		return true;					// swap opponent's opening move
	}
	else if (Last.j == 0)
	{
		DefendBlindRow(move);			// defend the blind row	
	}
	else
	{
		if (Last.i < N - Last.j)		// determine dual point on left
			move = Coordinates(N - Last.j, N - Last.i - 1);
		else							// determine dual point on right
			move = Coordinates(N - Last.j - 1, N - Last.i);
	}

	if (!IsValid(move) || Board[move.j][move.i])
		SpareMove(move);				// spare move - do some damage!

	return false;
}

// Determines best move along the blind row (row 0)
// Returns Whether appropriate defending move was found.

bool HEX_Game::DefendBlindRow(Coordinates& move) const
{
	move = Coordinates(-1, -1);		// should already be initialised but make sure

	for (int d = 0; d < NUM_DEFENSE_TEMPLATES; d++)
	{
		// Find root position H on top row
		for (int root = 0; root < 4; root++) {
			if (DefenseTemplate[d][0][root * 2] == 'H')
				break;
			if
				(
				root >= 4									// no root found
				||
				root == 0 && !(Last.i == 0 && Last.j == 0)	// acute corner no good	
				||
				root == 3 && !(Last.i == N - 1 && Last.j == 0)	// obtuse corner no good
				)
				continue;
				// Check whether rest of template matches board position
				bool valid = true;

				int	reentrant = 0;

				for (int j = 0; j < 2 && valid; j++)
				for (int c = 0; c < 4 && valid; c++)
				{
					int i = Last.i - root + c;
					char ch = DefenseTemplate[d][j][c * 2];

					if (ch == 'V' && !Board[j][i])
						move = Coordinates(i, j);
					else if (ch == 'V' && Board[j][i])
						valid = false;
					else if (ch == 'h' && Board[j][i] != HORZ)
						valid = false;
					else if (ch == 'v' && Board[j][i] != VERT)
						valid = false;
					else if (ch == 'x' && !Board[j][i])
						valid = false;
					else if (ch == '.' && Board[j][i])
						valid = false;
					else if (ch == 'r')
					{
						if (Board[j][i] == VERT)
							valid = false;
						else
						{
							move = Coordinates(Last);
							reentrant = (i < root) ? -1 : 1;  // is reentrant move
						}
					}
				}
					if (!valid)
						continue;		// invalid template

					if (move.i == -1 || move.j == -1)
						continue;		// no valid reply found

					if (reentrant)
					{
						if (reentrant > 0)
						{
							// Find reentrant block above
							while (move.i < N - 1)
							{
								if (!Board[0][move.i])
									return true;			// adjacent block above

								if (!Board[1][move.i])
								{
									move = Coordinates(move.i, 1);
									return true;			// reentrant block above
								}
								move.i++;
							}
						}
						else
						{
							// Find reentrant block below
							while (move.i >= 0)
							{
								if (!Board[0][move.i])
									return true;			// adjacent block above

								if (!Board[1][move.i - 1])
								{
									move = Coordinates(move.i - 1, 1);
									return true;			// reentrant block above
								}
								move.i--;
							}
						}
						return false;						//	no reentrant block found
					}
					return  true;	// good move found!
			}
			return false;
		}
		

		

		
}


// Tells best spare move. 

void HEX_Game::SpareMove(Coordinates& move) const
{
	// Look for urgent moves on the blind row
	move.j = 0;
	for (move.i = 0; move.i < N; move.i++)
	if
		(
		!Board[0][move.i]
		&&
		(
		move.i < N - 1 && Board[0][move.i + 1] == HORZ
		&&
		(
		Board[1][move.i] == VERT
		||
		move.i > 0 && !Board[1][move.i] && Board[1][move.i - 1] == HORZ
		)
		||
		move.i > 0 && Board[0][move.i - 1] == HORZ
		&&
		(
		Hex_board[1][move.i - 1] == VERT
		||
		!Hex_board[1][move.i] && Hex_board[1][move.i + 1] == HORIZ
		)
		)
		)
		return;

	// top obtuse corner = empty, use it
	if (!Hex_board[0][N - 1])
	{
		move = Coordinates(N - 1, 0);
		return;
	}

	// stop any re-entred point on row 1
	move.j = 1;
	for (move.i = 0; move.i < N - 1; move.i++)		// skip move.i == N - 1
	if
		(
		!Hex_board[1][move.i]
		&&
		(
		Board[0][move.i] == HORIZ && Board[0][move.i + 1] != HORIZ
		||
		Board[0][move.i + 1] == HORIZ && Board[0][move.i] != HORIZ
		)
		)
		return;

	// Block any point adjacent to white on row 0
	move.j = 0;
	for (move.i = 0; move.i < N; move.i++)
	if
		(
		!Board[0][move.i]
		&&
		(
		move.i > 0 && Board[0][move.i - 1] == HORIZ
		||
		move.i < N - 1 && Board[0][move.i + 1] == HORIZ
		)
		)
		return;

	// point an empty point away from black on row 0
	move.j = 0;
	for (move.i = 0; move.i < N; move.i++)
	if
		(
		!Board[0][move.i]
		&&
		(
		move.i > 1 && !Board[0][move.i - 1] && Board[0][move.i - 2] == VERT
		||
		move.i < N - 2 && !Board[0][move.i + 1] && Board[0][move.i + 2] == VERT
		)
		)
		return;

	// point from top obtuse corner down (at least one)
	for (move.j = 0; move.j < N; move.j++)
	for (move.i = N - 1; move.i >= 0; move.i--)
	if (!Board[move.j][move.i])
		return;

	move = Coordinates(-1, -1);	//bad 
}

int main(int argc, char* argv[])
{
	cout << "Hex_board " << endl;
	cout << "user horizontal (H), computer is vertical (V)" << endl;

	int n = 11;
	if (argc == 2)
	{
		n = atoi(argv[1]);
		if (n < MIN_N)
			n = MIN_N;
		else if (n > MAX_N)
			n = MAX_N;
	}

	HEX_Game game(n);
	while (game.Play());
}
